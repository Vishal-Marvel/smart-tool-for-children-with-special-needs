...
import dummy from "@/public/memory/dummpy-removebg-preview.png";
...
export const MemoryGame = ({ id }: Props) => {
  ...
  useEffect(() => {
    if (gameLev == 1 && matchedCards.length === 4) {
      setGameOver(true);
    } else if (gameLev == 2 && matchedCards.length === 8) {
      setGameOver(true);
    } else if (gameLev == 3 && matchedCards.length === 16) {
      setGameOver(true);
    }
  }, [moves]);
  ...
  const shuffle = () => {
    ...
    if (gameLev === 1) {
      shuffledCards = [
        ...board.slice(random2, random2 + 2),
        ...board.slice(random2, random2 + 2),
      ];
    } else if (gameLev === 2) {
      shuffledCards = [...board.slice(0, 4), ...board.slice(0, 4), dummy];
    } else if (gameLev === 3) {
      shuffledCards = [...board, ...board];
    }

    shuffledCards.sort(() => Math.random() - 0.5).map((v) => v);
  ...
  const completeGame = () => {
    if (gameOver === true) {
      sound.play();
      const maximum = gameLev == 1 ? 4 : gameLev === 2 ? 8 : 16;
      let accuracy;
      if (moves <= maximum * 1.5) accuracy = 5;
      else if (moves <= maximum * 2) accuracy = 4;
      else if (moves <= maximum * 2.5) accuracy = 3;
      else if (moves <= maximum * 3) accuracy = 2;
      else accuracy = 1;
    ...
    <div className={" flex flex-col items-center justify-evenly h-full"}>
      ...
      {instruction && (
        <GameInstruction
          ...
          instructions={[
            "You need to click on the circle to see the object",
      ...
      {!instruction && (
        <>
          <span className={"text-center font-semibold text-2xl capitalize"}>
            PAIR THE HIDDEN OBJECT
          </span>
          <div className="text-center">
            ...
            <span
              className={
                "text-indigo-950 dark:text-indigo-50 text-2xl font-bold uppercase "
              }
            >
              {" "}
              Level - {gameLev}
