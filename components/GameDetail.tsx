...
export const GameDetail = ({ game, userId }: Props) => {
  ...
  useEffect(() => {
    ...
    const accuracies = sortedGameData.map(
      (data) => (data.accuracy / data.maximum) * 100
    );
    ...
    const avgAccuracy =
      accuracies.length > 0
        ? accuracies.reduce((acc, accuracy) => acc + accuracy, 0) /
          accuracies.length
        : 0;
  ...
  return (
    <div className="">
      {gameData.length > 0 && (
        <div
          className={
            "w-[97%] bg-opacity-40 bg-white m-4 border-4 shadow-2xl border-emerald-600 rounded-2xl p-2 text-center md:text-left flex flex-col"
          }
        >
          <span
            className={
              "text-2xl font-bold uppercase md:ml-10 m-3  underline underline-offset-3"
            }
          >
            {game.name}
          </span>
          ...
          <div
            className={
              "w-full flex justify-center md:p-8 md:m-4 mt-1 mb-1 overflow-hidden"
            }
          >
            <Table
              className={
                "md:w-2/3 w-1/3 shadow-xl   rounded-2xl bg-blue-300 bg-blend-color bg-opacity-30"
              }
            >
              <TableHeader className={""}>
                <TableRow
                  className={
                    "bg-secondary bg-opacity-30 rounded-2xl divide-x divide-gray-200"
                  }
                >
              ...
              <TableBody>
                {gameData.map((game, index) => (
                  <TableRow className={"divide-x divide-gray-200"}>
                    ...
                    <TableCell>
                      {((game.accuracy / game.maximum) * 100).toFixed(0)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
          ...
          <div className={"w-full flex justify-center md:m-2 md:p-2"}>
            <div className={"text-center w-2/3 bg-gray-300 rounded-2xl p-4"}>
              <span className={"md:text-[16px] text-sm"}>
                {"The time-accuracy graph of the user states that the "}
